...
public class SudokuBoard {
    ...
    public boolean isValid() {
        return checkCharacter() && checkRowDuplicate() &&
            checkColumnDuplicate() && checkMiniSquareDuplicate();
    }
        ...
        for (int spot = 1; spot <= 9; spot++) {
            ...
            for (int r = 0; r < miniSquare.length; r++) {
                for (int c = 0; c < miniSquare[r].length; c++) {
                    if (gridValue.contains(miniSquare[r][c]) && miniSquare[
                            r][c] != '-') {
                        return false;
                    }
        ...
        for (int r = 0; r < board.length; r++) {
            for (int c = 0; c < board[r].length; c++) {
                if (!boardMap.containsKey(board[r][c])) {
                    ...
                    boardMap.put(board[r][c], boardMap.get(board[r][c]) +
                    1);
                }
            }
        }
